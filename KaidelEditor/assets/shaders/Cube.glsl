...
vec4 ApplyLighting(vec4 objectColor){
	...
	for(int i = 0;i<1;++i){	
		...
		//Ambient
		totalAmbient += u_Lights[i].AmbientStrength * lightColor;

		//Diffuse
		totalDiffuse += max(dot(norm, lightDir), 0.0)* lightColor;

		//Specular
		vec3 viewDir = normalize(u_CameraPosition - Input.FragPos);
		vec3 reflectDir = reflect(-lightDir,norm);
		totalSpecular += 0.5*pow(max(dot(viewDir,reflectDir),0.0),32) * lightColor;
	}
