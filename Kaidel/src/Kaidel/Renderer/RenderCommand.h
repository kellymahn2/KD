...
namespace Kaidel {
	...
	public:
		...
		static void BindPushConstants(Ref<Shader> shader, uint32_t firstIndex, const uint8_t* values, uint64_t size) {
			s_RendererAPI->BindPushConstants(shader, firstIndex, values, size);
		}
		...
		static void DrawIndexed(uint32_t indexCount, uint32_t vertexCount, uint32_t instanceCount, uint32_t indexOffset, uint32_t vertexOffset, uint32_t instanceOffset) {
			s_RendererAPI->DrawIndexed(indexCount, vertexCount, instanceCount, instanceOffset, vertexOffset, instanceOffset);
		}
