...
namespace Kaidel {
	...
	static void SetCubeVertexValues(CubeVertex& cv, const glm::mat4& transform, const glm::vec4& color, const glm::vec4& position, int entityID) {
	...
	void Renderer3D::DrawCube(const glm::mat4& transform, const glm::vec4& color, int entityID) {
		CubeVertex first{};
		SetCubeVertexValues(first, transform, color, s_Data.CubeVertexPositions[0], entityID);
		CubeVertex second{};
		SetCubeVertexValues(second, transform, color, s_Data.CubeVertexPositions[1], entityID);
		CubeVertex third{};
		SetCubeVertexValues(third, transform, color, s_Data.CubeVertexPositions[2], entityID);
		CubeVertex fourth{};
		SetCubeVertexValues(fourth, transform, color, s_Data.CubeVertexPositions[3], entityID);
		CubeVertex fifth{};
		SetCubeVertexValues(fifth, transform, color, s_Data.CubeVertexPositions[4], entityID);
		CubeVertex sixth{};
		SetCubeVertexValues(sixth, transform, color, s_Data.CubeVertexPositions[5], entityID);
		CubeVertex seventh{};
		SetCubeVertexValues(seventh, transform, color, s_Data.CubeVertexPositions[6], entityID);
		CubeVertex eighth{};
		SetCubeVertexValues(eighth, transform, color, s_Data.CubeVertexPositions[7], entityID);

		std::scoped_lock<std::mutex> lock(s_Data.CubeMutex);

		auto bvi = s_Data.CubeVertexBufferArray.Reserve(8);
		bvi[0] = first;
		bvi[1] = second;
		bvi[2] = third;
		bvi[3] = fourth;
