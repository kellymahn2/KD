...
namespace Kaidel {
	...
	namespace Utils {
		static VkPipeline CreatePipeline(const GraphicsPipelineSpecification& specs)
		{
			...
			return VK_CONTEXT.GetBackend()->CreateGraphicsPipeline(
				shaderInfo,
				vertexInputState,
	...
	{
		m_Pipeline = Utils::CreatePipeline(specs);
		m_Specification.Shader->BindPipeline(this);
	}
	...
	void VulkanGraphicsPipeline::Recreate()
	{
		...
		m_Pipeline = Utils::CreatePipeline(m_Specification);
	}
